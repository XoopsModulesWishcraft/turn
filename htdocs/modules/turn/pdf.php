...
require_once($GLOBAL['xoops']->path(_MI_TURN_FRAMEWORK_TCPF));
...
if(empty($_POST["pdf_data"])) {
	...
	$post_handler =& xoops_getmodulehandler('books', 'turn');
	...
	ob_start();
	$GLOBAL['xoopsTpl']->display('db:turn_pdf.html');
...
$pdf_data['title'] = _MN_TURN_PDF_SUBJECT.': '.$pdf_data["title"];
$pdf_data['author'] = _MN_TURN_PDF_AUTHOR.': '.$pdf_data['author'];
$pdf_data['date'] = _MN_TURN_PDF_DATE. ': '.date(_DATESTRING, $pdf_data['date']);
$pdf_data['url'] = _MN_TURN_PDF_URL. ': '.$pdf_data['url'];
