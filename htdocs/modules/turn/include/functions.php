<?php
if (!function_exists('turn_getFilterElement')) {
	function turn_getFilterElement($filter, $field, $sort='created', $op = '', $fct = '') {
		$components = turn_getFilterURLComponents($filter, $field, $sort);
		$ele = false;
		include_once('formobjects.turn.php');
		switch ($field) {
			case 'bid':
				$ele = new turnFormSelectBooks('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	...
		    	break;
			case 'cid':
				$ele = new turnFormSelectChapters('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
		    	break;			
			case 'mode':
				if ($op=='css') {
					$ele = new turnFormSelectCSSMode('', 'filter_'.$field.'', $components['value'], 1, false, true);
			    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
			    	break;
				} elseif ($op=='pages') {
					$ele = new turnFormSelectPagesMode('', 'filter_'.$field.'', $components['value'], 1, false, true);
			    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
			    	break;
				}
			case 'filetype':
				$ele = new turnFormSelectPagesFiletype('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
		    	break;						
		    case 'language':
				$ele = new turnFormSelectLanguage('', 'filter_'.$field.'', $components['value'], 1, false, true);
		    	$ele->setExtra('onchange="window.open(\''.$_SERVER['PHP_SELF'].'?'.$components['extra'].'&filter='.$components['filter'].(!empty($components['filter'])?'|':'').$field.',\'+this.options[this.selectedIndex].value'.(!empty($components['operator'])?'+\','.$components['operator'].'\'':'').',\'_self\')"');
		    	break;
		    case 'description':
		    case 'name':
...
if (!function_exists('turn_getFilterURLComponents')) {
	function turn_getFilterURLComponents($filter, $field, $sort='created') {
    	...
    	$pagenav = array();
    	$pagenav['op'] = isset($_REQUEST['op'])?$_REQUEST['op']:"turn";
...
if (!function_exists("turn_adminMenu")) {
  function turn_adminMenu ($currentoption = 0)  {
		$module_handler = xoops_gethandler('module');
		$xoModule = $module_handler->getByDirname('turn');
  ...
  function turn_footer_adminMenu()
  {
		echo "</div></td></tr>";
  		echo "</table>";
  }
