 ...
 * @package kernel
 */
class turnPages extends XoopsObject
{
    ...
    function turnPages($id = null)
    {
        ...
        $this->initVar('mode', XOBJ_DTYPE_ENUM, '_MI_TURN_MODE_HTML', false, false, false, array('_MI_TURN_MODE_HTML','_MI_TURN_MODE_LANDSCAPE','_MI_TURN_MODE_PORTTRAIT'));
        $this->initVar('html', XOBJ_DTYPE_OTHER, '', false);
    ...
    function getImageURL() {
    	$url = XOOPS_URL.'/modules/'._MI_TURN_DIRNAME.'/pages/page-'.$this->getVar('bid').'-'.$this->getVar('pid').'-'.$this->getVar('page');
	...
	function getPrintURL() {
    	$url = XOOPS_URL.'/modules/'._MI_TURN_DIRNAME.'/pages/print/page-'.$this->getVar('bid').'-'.$this->getVar('pid').'-'.$this->getVar('page');
    	switch($this->getVar('filetype')) {
    		default:
    		case '_MI_TURN_FILETYPE_JPG':
    			$url .= '.jpg';
    ...
    function getZoomURL() {
    	$url = XOOPS_URL.'/modules/'._MI_TURN_DIRNAME.'/pages/large/page-'.$this->getVar('bid').'-'.$this->getVar('pid').'-'.$this->getVar('page');
    	switch($this->getVar('filetype')) {
    		default:
    		case '_MI_TURN_FILETYPE_JPG':
    			$url .= '.jpg';
    ...
    function getPDFURL() {
    	$url = XOOPS_URL.'/modules/'._MI_TURN_DIRNAME.'/pages/pdf/page-'.$this->getVar('bid').'-'.$this->getVar('pid').'-'.$this->getVar('page');
    	switch($this->getVar('filetype')) {
    		default:
    		case '_MI_TURN_FILETYPE_JPG':
    			$url .= '.jpg';
    ...
    function getForm() {
    	return turn_pages_get_form($this, false);
    }
    ...
    function toArray() {
    	...
    	$ele = turn_pages_get_form($this, true);
    	...
    	switch ($this->getVar('filetype')) {
			...
			case '_MI_TURN_FILETYPE_PNG':
			case '_MI_TURN_FILETYPE_GIF':
				$ret['thumbnail'] = XOOPS_URL.'/modules/turn/thumbnail.php?pid='.$this->getVar('pid');
				break;	
    	}
	...
	function runInsertPlugin() {
		...
		include_once($GLOBALS['xoops']->path('/modules/turn/plugins/'.constant($this->getVar('filetype').'_PLUGINFILE').'.php'));
...
* @package kernel
*/
class turnPagesHandler extends XoopsPersistableObjectHandler
{
	...
	function __construct(&$db) 
    {
    	...
    	$this->db = $db;
        parent::__construct($db, 'turn_pages', 'turnPages', "pid", "filetype");
    }
 	...
 	private function runGetArrayPlugin($row) {
		...
		include_once($GLOBALS['xoops']->path('/modules/turn/plugins/'.constant($row['filetype'].'_PLUGINFILE').'.php'));
	...
	function insert($obj, $force=true, $run_plugin = false) {
		...
		$this->_bHandler = xoops_getmodulehandler('books', 'turn');
    	...
    	if ($obj->vars['width']['changed']==true&&$obj->vars['height']['changed']==true) {
	    	if ($obj->getVar('width')>$obj->getVar('height')) {
	    		$obj->setVar('mode', '_MI_TURN_MODE_LANDSCAPE');
	    		$obj->setVar('wide', '_MI_TURN_TRUE');
	    	} else {
	    		$obj->setVar('mode', '_MI_TURN_MODE_PORTTRAIT');
	    	}
    	}
    	
    	switch($obj->getVar('extension')) {
    		case 'swf':
    			$obj->setVar('filetype', '_MI_TURN_FILETYPE_SWF');
    			break;
    		case 'gif':
    			$obj->setVar('filetype', '_MI_TURN_FILETYPE_GIF');
    			break;
    		case 'png':
    			$obj->setVar('filetype', '_MI_TURN_FILETYPE_PNG');
    			break;
    		default:
    		case 'jpg':
    		case 'jpeg':
    			$obj->setVar('filetype', '_MI_TURN_FILETYPE_JPG');
    			break;
    	}
    	
    	if ($obj->vars['path']['changed']==true) {
	    	if(strpos( ' '. $obj->getVar('path'), XOOPS_UPLOAD_PATH)>0) {
	    		$obj->setVar('location', '_MI_TURN_PATH_XOOPS_UPLOAD_PATH');
	    		$obj->setVar('path',  str_replace(XOOPS_UPLOAD_PATH, '',  $obj->getVar('path')));
	    	} elseif(strpos( ' '. $obj->getVar('path'), XOOPS_VAR_PATH)>0) {
	    		$obj->setVar('location', '_MI_TURN_PATH_XOOPS_VAR_PATH');
	    		$obj->setVar('path',  str_replace(XOOPS_VAR_PATH, '',  $obj->getVar('path')));
	    	} else {
	    		$obj->setVar('location', '_MI_TURN_PATH_OTHER');
	    	}
    	}
